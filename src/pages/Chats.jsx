...
export default function Chats() {
  ...
  const onCreateChat = async (data) => {
    try {
      ...
      setFilteredChats(previousChats => [newChat, ...previousChats]);
  
      setCurrentChat({
        id: newChat.chat_id,
        name: newChat.name || data.newChatUsername,
        lastActivity: 'last seen just now'
      });
  
      localStorage.setItem('currentChatName', newChat.name || data.newChatUsername);
  
      setActive(newChat.chat_id);
